...
def calcular_valor():
    try:
        ...
        # Cálculo do valor da Chave Pix (Valor Total + 1%)
        valor_chave_pix = valor_total * 1.01
        ...
        resultado = (
            ...
            f"Multa por Atraso (2%): R$ {multa_por_atraso:.2f}\n"
            f"Juros por Dias de Atraso: R$ {multa_dias_atraso:.2f}\n" # Renomeado para clareza
            f"Taxa de Reemissão de Boleto: R$ {taxa_reemissao_boleto:.2f}\n"
            f"\nValor Total a Pagar (Boleto): R$ {valor_total:.2f}\n" # Clarificando que é o valor do boleto
            # --- NOVA LINHA NO RESULTADO ---
            f"\nChave Pix (Total + 1%): R$ {valor_chave_pix:.2f}"
            # --- FIM NOVA LINHA ---
        )
